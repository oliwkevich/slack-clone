...
const generateCode = () => {
	const code = Array.from(
		{ length: 6 },
		() => '0123456789qwertyuiopasdfghklzxcvbnm'[Math.floor(Math.random() * 36)]
	).join('');
...
export const join = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (existingMember) throw Error('Already an member');
...
export const get = query({
	...
	handler: async ctx => {
		...
		const workspacesIds = members.map(el => el.workspaceId);
		const workspaces = [];
